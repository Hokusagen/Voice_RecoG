...
class HotkeysConfig:
    ...
    correct: str = "ctrl+shift+f8"
    """Запомнить правку: выделить исправленный текст и нажать.

    Не удерживается, а нажимается. Приложение скопирует выделенное, найдёт, из
    какой диктовки оно выросло, и запишет в журнал одни различия — по ним потом
    видно слово, которое не разобрал ни Whisper, ни редактор.
    """
...
class CloudConfig:
    ...
    output_tokens_per_minute: int = 1000
    """Лимит Groq на выходные токены в минуту (OTPM).

    В заголовках ответа его нет, а считается по нему не фактический ответ, а
    заявленный max_tokens: запрос с потолком выше лимита сервер отбивает
    мгновенно, не доходя до модели. Поэтому потолок ответа мы считаем по длине
    фразы, а расход держим в уме между диктовками. У другого сервера лимит
    свой; 0 — не ограничивать.
    """
